// O(n*m) algo to compute edge coloring of general graph in maxdeg+1 colors
// Algo logic: https://en.wikipedia.org/wiki/Vizing%27s_theorem#Algorithms
const int MAXN=100+5;
namespace Vizing{ // returns edge coloring in adjacent matrix G. 1 - based
	int C[MAXN][MAXN], G[MAXN][MAXN];
	void clear(int N){
		for(int i=0; i<=N; i++) for(int j=0; j<=N; j++) C[i][j] = G[i][j] = 0;
	}
	void solve(vector<p32> &E, int N){
		int X[MAXN] = {}, a;
